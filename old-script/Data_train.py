...
# === 1. Fonction Open-Meteo (2010–2023) ===
def fetch_open_meteo(lat, lon):
    ...
    print("📱 Téléchargement des données Open-Meteo (2010–2023)…")
...
if __name__ == "__main__":
    ...
    print("=== Génération des données d'entraînement avec score de production agricole ===")
    df = fetch_open_meteo(LAT, LON)
    df["agri_score"] = df.apply(compute_agri_score, axis=1)
    df.to_csv("train_with_score.csv", index=False)
    print("✅ Fichier généré : train_with_score.csv avec colonne agri_score")
